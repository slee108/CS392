   ...
   for (int fork_index = 0; fork_index < num_forks; ++fork_index){
    ...
    /*
      ...
      Error: wait() failed. %s.
         %s will be strerror(errno)
    */
   for(int fork = spawned; fork > 0; fork--){
        int whereIsMyPid;
        int pidder = wait(&whereIsMyPid);
        if (pidder >= 0){
